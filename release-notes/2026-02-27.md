...
**外部 Secrets 管理**讓使用者終於可以將 API key、token 等敏感資訊從 `openclaw.json` 中分離出去，透過 `env`、`file`、`exec` 三種 provider 對接任意外部金鑰管理系統（如 Vault、AWS Secrets Manager、1Password），並支援執行期熱重載，不需重啟 gateway 即可輪換憑證。

**ACP 執行緒代理**升格為一等公民執行環境，代理現在可以在執行緒 session 中完整運作，擁有獨立的生命週期管理與自動恢復機制，多個代理的回覆也會自動合併，大幅降低通道噪音，適合需要多代理協作的複雜場景。

**Agents 路由 CLI** 新增 `openclaw agents bind/unbind/bindings` 三個子命令，讓使用者可以精確控制哪個 agent 負責哪個通道帳號的訊息，支援角色感知的身份綁定與插件解析，並可在新增通道時直接提示綁定，大幅簡化多 agent 多通道的管理流程。
